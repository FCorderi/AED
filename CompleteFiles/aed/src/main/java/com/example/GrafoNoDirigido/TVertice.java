...
public class TVertice<T> implements IVertice, IVerticeKevinBacon {
    ...
    public void bea(Collection<TVertice> visitados) {
        ...
        LinkedList<TVertice> C = new LinkedList<>();
        C.addLast(this);
        visitados.add(this);
        while(!C.isEmpty()){
            TVertice vertice = C.removeLast();
    ...
    public void numBacon(Comparable actor){
        this.setVisitado(true);
        LinkedList<TVertice> C = new LinkedList<>();
        C.addLast(this);
        
        while(!C.isEmpty()){
            TVertice vertice = (TVertice) C.removeLast();
            LinkedList<TAdyacencia> adyacentes  = vertice.getAdyacentes();
            for(TAdyacencia a : adyacentes){
                ...
                if (!destino.getVisitado()){
                    destino.setVisitado(true);
                    C.addLast(destino);
                    destino.setBacon(vertice.numBacon + 1);
                }
            }
        }   
    }
